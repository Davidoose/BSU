...
int main() {
	...
	thread_info* data = new thread_info[number_of_threads];
	...
	for (int i = 0; i < number_of_threads; ++i) {
		data[i].arr = arr;
		data[i].array_size = size;
		data[i].thread_index = i + 1;
		data[i].start_work = start_work;
		stopped_threads[i] = data[i].stop_work = CreateEvent(NULL, TRUE, FALSE, NULL);
		data[i].terminate_or_continue = new HANDLE[2];
		data[i].terminate_or_continue[0] = CreateEvent(NULL, FALSE, FALSE, NULL);
		data[i].terminate_or_continue[1] = CreateEvent(NULL, FALSE, FALSE, NULL);
		threads[i] = CreateThread(NULL, 0, thread_func, (LPVOID)(&data[i]), NULL, NULL);
	...
	while (ended_threads != number_of_threads) {
		...
		while (!is_thread_terminated) {
			...
			if (terminated_threads[thread_to_terminate_ind]) {
				...
				SetEvent(data[thread_to_terminate_ind].terminate_or_continue[0]);
		...
		for (int j = 0; j < number_of_threads; ++j) {
			if (!terminated_threads[j]) {
				ResetEvent(data[j].stop_work);
				SetEvent(data[j].terminate_or_continue[1]);
			}
		}
	...
	for (int i = 0; i < number_of_threads; ++i) {
		...
		CloseHandle(data[i].terminate_or_continue[0]);
		CloseHandle(data[i].terminate_or_continue[1]);
	}
	delete[] threads;
	delete[] stopped_threads;
	for (int i = 0; i < number_of_threads; ++i) {
		delete[] data[i].terminate_or_continue;
	}
	delete[] data;
